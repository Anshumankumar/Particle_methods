...
def cubicSpline(q,h):
    if (q>2):
        ...
        return 2/(12*h)*math.pow(2-q,3)
    else:
        return 2/(3*h)*(1-3/2*pow(q,2)*(1-q/2))
...
def cubicSplineD(q,h):
    ...
    if (abs(q) > 1 and abs(q)<=2):
        return  -1/(2*pow(h,2))*pow(2-abs(q),2)*q/abs(q)
    else:
        return  2/(3*h)*( - 3*abs(q)/h +9/4*pow(q,2)/h)*q/abs(q)
...
class Particle:
    ...
    def computeAVis(self,p):
        ...
        cab = (ca+cb)/2
        muab = hab*vab*rab/(pow(rab,2)+2)
        rhoab = (self.pressure+p.pressure)/2
